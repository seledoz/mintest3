...
window.__minibiaBotBundle.installSpellTimerModule = function installSpellTimerModule(bot) {
  ...
  function getPlayerPosition() {
    return normalizePosition(
      bot.getPlayerPosition?.() || window.gameClient?.player?.getPosition?.()
    );
  }
  ...
  function getMonstersWithinRange() {
    ...
    return monsters.filter((monster) => {
      ...
      return getTileDistance(playerPosition, monsterPosition) <= config.monsterGuardRange;
    });
  }

  function isMonsterBlocked() {
    return getMonstersWithinRange().length > 0;
  }

  function resetTimer(now = Date.now()) {
    state.due = false;
    state.nextCastAt = now + config.intervalSeconds * 1000;
  }
  ...
  function updateConfig(next = {}) {
    ...
    if (Object.prototype.hasOwnProperty.call(next, "hotbarSlot")) {
      config.hotbarSlot = normalizeSlot(next.hotbarSlot) || config.hotbarSlot;
    }
    if (Object.prototype.hasOwnProperty.call(next, "enabled")) {
      config.enabled = !!next.enabled;
      if (config.enabled) start();
      else stop({ persistEnabled: true });
    }
    persistConfig();
    syncUi();
  }
  ...
  function installUi() {
    ...
    section.innerHTML = `
      ...
      <div class="mb-stack">
        <label class="mb-toggle">
          <input type="checkbox" id="minibia-bot-spell-timer-enabled" />
          <span>Enable Spell Timer</span>
        </label>
        <label class="mb-field">
          <span class="mb-field-label">Timer (seconds)</span>
          <input type="number" id="minibia-bot-spell-timer-interval" min="0.1" step="0.1" />
        </label>
        <label class="mb-field">
          <span class="mb-field-label">Hotkey</span>
          <input type="number" id="minibia-bot-spell-timer-hotkey" min="1" max="12" step="1" />
        </label>
    ...
    section.querySelector("#minibia-bot-spell-timer-enabled")?.addEventListener("change", (event) => {
      updateConfig({ enabled: event.target.checked });
    });
    section.querySelector("#minibia-bot-spell-timer-interval")?.addEventListener("change", (event) => {
      updateConfig({ intervalSeconds: event.target.value });
    });
    section.querySelector("#minibia-bot-spell-timer-hotkey")?.addEventListener("change", (event) => {
      updateConfig({ hotbarSlot: event.target.value });
    });
    syncUi();
    return true;
  }
